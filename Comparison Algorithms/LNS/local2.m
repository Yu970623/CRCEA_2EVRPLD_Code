...
function route = move2(vrp2e,route)
    ...
    for c = seq_c
        ...
        %----计算原客户的代价----
        cost_init = funcopt3(vrp2e,route);
        ...
        cost_front = arrayfun(@(k) [funcopt3(vrp2e,c_front{k})], 1:length(c_front), 'UniformOutput', false);
        % 计算插入到 rback 后的新路径
        c_back = arrayfun(@(k) [seq_r(1:k), c, seq_r(k+1:end)], pos_in, 'UniformOutput', false);
        cost_back = arrayfun(@(k) [funcopt3(vrp2e,c_back{k})], 1:length(c_back), 'UniformOutput', false);
...
function fvrp = funcopt2(vrp2e,route)
    fleet = vrp2e.fleet;
    dis_sc = vrp2e.dis_sc;
    cap_sc = energy(vrp2e,route(1:end-1));  %当前子路段的实时运载重量
